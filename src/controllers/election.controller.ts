    ...
    get(DNAVOTE_API_HOST + '/api/dna-selection/v1/period/simple-info')
      ...
      .then(([height, candidates]) => {
        ...
        res.json(new ResponseSuccess({
          ...
          secondaryRevoteEnabled: SECONDARY_REVOTE_ENABLED,
          secondaryRevoteStartHeight: SECONDARY_ELECTION_REVOTE_START,
          secondaryRevoteEndHeight: SECONDARY_ELECTION_REVOTE_END,
          secondaryRevoteEndTime: SECONDARY_REVOTE_ENABLED_UNTIL,
          previousSecondaryVoteEndHeight: PREVIOUS_SECONDARY_VOTE_END,
        }))
      }).catch((err) => {
        console.error(err)
